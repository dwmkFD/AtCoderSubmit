...
template<typename T = ll> mint comb( T n, T r, T m = MOD )
{
	if ( n < r ) return ( 0 );
	if ( n < 0 || r < 0 ) return ( 0 );
	if ( r == 0 ) return ( 1 );
	...
	mint f = 1; for ( ll x = 0; x < r; ++x ) f *= ( ( n - x ) % MOD );
	mint t = 1; for ( ll x = 2; x <= r; ++x ) t *= ( x % MOD );
	t = modpow( t, MOD - 2 );
...
int main()
{
	...
	if ( 2 * k >= n )
		cout << comb( n + n - 1, n ) << endl;
	else
	{
		mint x = comb( n, k );
		mint y = n; y = modpow( y, k );
