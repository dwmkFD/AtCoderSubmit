...
int main()
{
	...
	vector<vector<mint>> dpsum( h + 1, vector<mint>( w + 1 ) );
	vector<vector<mint>> dpsum2( h + 1, vector<mint>( w + 1 ) );
	rep( i, h )
	{
		rep( j, w )
		{
			dpsum[i + 1][j + 1] = dpsum[i][j + 1] + dpsum[i + 1][j] - dpsum[i][j] + dp[i][j];
		}
	}
	rep( i, h )
	{
		rep( j, w )
		{
			dpsum2[i + 1][j + 1] = dpsum2[i][j + 1] + dpsum2[i + 1][j] - dpsum2[i][j] + dpsum[i][j];
		}
	}

	rep( i, h + 1 ) rep( j, w + 1 ) cout << dpsum[i][j] << " \n"[j == w];
	rep( i, h + 1 ) rep( j, w + 1 ) cout << dpsum2[i][j] << " \n"[j == w];

	cout << dpsum2[h][w] << endl;
